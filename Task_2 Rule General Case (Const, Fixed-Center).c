...
int main(int argc, char *argv[])
{
  ...
  if (id==MASTER)
   {  
       if(argc != 4 ){
                printf("Must be entered 3 paramets:\n");
                printf("   1st: Rule number (0-255)\n");
                printf("   2nd: Size of array\n");
                printf("   3rd: number of interation\n");
                //MPI_Finalize();
                exit(0);
                }
    }
...
{   
   ...
   for (i=0;i<=7;i++) 
    ...
    a[0]=0; a[Arr_size+1]=0; 
    
    //generating random initial ODCAA
    for (i=1; i<=Arr_size; i++) {a[i]= 0;} a[Arr_size/2]= 1;
